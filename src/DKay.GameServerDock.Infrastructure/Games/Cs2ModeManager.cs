...
public sealed partial class Cs2ModeManager : ICs2ModeManager
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var practiceMode = Cs2ModeCatalog.ResolvePracticeMode(preset, request.PracticeMode);
    ...
    public async Task<Cs2ModeProfile> SetActivePracticeModeAsync(
        GameServerInstance server,
        string practiceMode,
        CancellationToken cancellationToken)
    {
        ...
        profile = NormalizeProfile(profile, preset) with
        {
            PracticeMode = Cs2ModeCatalog.ResolvePracticeMode(preset, practiceMode),
            UpdatedAt = DateTimeOffset.UtcNow
        };
        var convars = BuildProfileConVars(profile, preset);
        await WriteProfileConfigurationAsync(server, profile, preset, convars, cancellationToken);
    ...
    private static Cs2ModeProfile NormalizeProfile(
        ...
        profile with
        {
            ...
            PracticeMode = Cs2ModeCatalog.ResolvePracticeMode(preset, profile.PracticeMode)
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        var practiceMode = Cs2ModeCatalog.ResolvePracticeMode(preset, profile.PracticeMode);
        var lines = new List<string>
        {
            "// Generated by DKay Game Server Dock. Applied after map and plugin configuration.",
            $"// Combat: {combatMode}; ammunition: {ammoMode}; respawn: {respawnMode}; SharpTimer HUD: {hudMode}; in-game practice: {practiceMode}"
        };
        ...
        if (sharpTimerInstalled)
        {
            ...
            lines.AddRange(Cs2ModeCatalog.BuildSharpTimerHudCommands(hudMode)
                .OrderBy(pair => pair.Key, StringComparer.Ordinal)
                .Select(pair => $"{pair.Key} {FormatCfgValue(pair.Value)}"));
            lines.AddRange(Cs2ModeCatalog.BuildSharpTimerPracticeCommands(practiceMode)
                .OrderBy(pair => pair.Key, StringComparer.Ordinal)
                .Select(pair => $"{pair.Key} {FormatCfgValue(pair.Value)}"));
        }
