...
7. Start the server. The generated `dkay-workshop-start.cfg` asks CS2 to download or update the selected item after Source2 has entered its console loop, build its map group and host it. The live console shows `DKAY_WORKSHOP_REQUEST <id>` before the request; leave the process running while UGC output is active. The saved profile changes from **downloads on start** to **installed** once a map payload exists on disk.
