...
7. Start the server. CS2 first opens the configured stock map as a short bootstrap so Steam, networking and RCON are initialized. The generated `dkay-workshop-start.cfg` then asks CS2 to download or update the selected item, build its map group and host it. The live console shows `DKAY_WORKSHOP_REQUEST <id>` before the request; leave the process running while UGC output is active. The saved profile changes from **downloads on start** to **installed** once a map payload exists on disk.
